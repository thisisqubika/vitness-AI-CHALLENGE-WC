 ...
 * target project's `.claude/settings.json` for the Expo ecosystem (the single
 * canonical `expo` plugin bundles all 13 Expo sub-skills). Safe to re-run.
 *
 * Usage:
...
function main() {
  ...
  if (!changed) {
    console.log(`No changes — Expo marketplace already registered in ${settingsPath}.`);
    return;
  }
  ...
  console.log('Restart Claude Code in this project; you will be prompted to trust the marketplace and install the expo plugin.');
}
